...
export const TenantForm: React.FC<TenantFormProps> = ({ onBack }) => {
  ...
  return (
    <div className="flex flex-col relative font-sans bg-white/60 backdrop-blur-xl rounded-[2.5rem] border border-white/60 shadow-2xl overflow-hidden animate-fade-in-up">
      <div className="relative z-20 bg-white/80 backdrop-blur-lg border-b border-white/50 px-8 py-6 overflow-hidden">
         {/* Watermark */}
         ...
         <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-6 mb-8 relative z-10">
             <div className="flex items-center gap-6">
                 ...
                 <div className="flex items-center gap-5">
                    <div className="w-20 h-20 rounded-2xl flex items-center justify-center shadow-lg shadow-[#1E3A8A]/20 bg-[#1E3A8A] text-white ring-4 ring-white">
                        <Store size={40} strokeWidth={1.5} />
                    </div>
                    <div>
                        <h1 className="text-3xl font-bold text-[#1E3A8A] font-playfair leading-tight">Potensi Ekonomi (Tenant)</h1>
                        <div className="flex items-center gap-2">
                             <span className="px-2.5 py-1 rounded-full bg-[#1E3A8A]/10 border border-[#1E3A8A]/20 text-xs font-bold text-[#1E3A8A] uppercase tracking-widest">Survei Hotel & Toko</span>
                        </div>
                    </div>
                 </div>
             </div>
...
const PremiumInput = ({ label, icon: Icon, type = "text", value, onChange, placeholder }: any) => (
  <div className="flex flex-col gap-2 group">
    <label className="text-xs font-bold text-gray-400 uppercase tracking-widest flex items-center gap-2 group-focus-within:text-[#1E3A8A] transition-colors">
    ...
    <input 
        ...
        className="w-full text-base font-semibold text-gray-700 bg-white border border-gray-200 rounded-xl px-4 py-3.5 focus:bg-white focus:border-[#1E3A8A] focus:ring-4 focus:ring-[#1E3A8A]/5 outline-none transition-all placeholder:text-gray-300" 
...
const CardInput = ({ icon: Icon, value, onChange, placeholder, type = "text", highlight = false }: any) => (
    <div className="relative group/input">
        ...
        <input 
            ...
            placeholder={placeholder}
            className={`w-full bg-white border border-gray-100 rounded-xl py-3 pl-10 text-sm font-semibold text-gray-700 focus:bg-white focus:border-[#1E3A8A] focus:ring-2 focus:ring-[#1E3A8A]/10 outline-none transition-all placeholder:text-gray-300 ${highlight ? 'text-[#1E3A8A]' : ''}`}
        />
    </div>
);
