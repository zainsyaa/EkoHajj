      ...
      <div className="relative z-20 bg-white/80 backdrop-blur-lg border-b border-white/50 px-4 py-4 md:px-8 md:py-6 overflow-hidden">
         ...
         <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4 md:gap-6 mb-6 md:mb-8 relative z-10">
             <div className="flex items-start md:items-center gap-4">
                 <button onClick={onBack} className="p-2 md:p-3 rounded-2xl hover:bg-white text-gray-500 hover:text-[#7C3AED] transition-all border border-transparent hover:border-gray-200 shrink-0 mt-1 md:mt-0"><ArrowLeft size={20} className="md:w-[22px] md:h-[22px]" /></button>
                 <div className="flex items-center gap-4">
                    <div className="w-14 h-14 md:w-20 md:h-20 rounded-2xl flex items-center justify-center shadow-lg shadow-[#7C3AED]/20 bg-[#7C3AED] text-white ring-4 ring-white shrink-0">
                        <Signal size={28} className="md:w-[40px] md:h-[40px]" strokeWidth={1.5} />
                    </div>
                    <div>
                        <h1 className="text-xl md:text-3xl font-bold text-[#7C3AED] font-playfair leading-tight mb-1">Telekomunikasi</h1>
                        <div className="flex items-center gap-2 flex-wrap">
                             <span className="px-2 py-0.5 md:px-2.5 md:py-1 rounded-full bg-[#7C3AED]/10 border border-[#7C3AED]/20 text-[10px] md:text-xs font-bold text-[#7C3AED] uppercase tracking-widest">Potensi Provider Jemaah</span>
                        </div>
                    </div>
                 </div>
             </div>
...
const PremiumInput = ({ label, icon: Icon, type = "text", value, onChange, placeholder }: any) => (
  <div className="flex flex-col gap-2 group">
    <label className="text-xs font-bold text-gray-400 uppercase tracking-widest flex items-center gap-2 group-focus-within:text-[#7C3AED] transition-colors">
    ...
    <input 
        ...
        className="w-full text-base font-semibold text-gray-700 bg-white border border-gray-200 rounded-xl px-4 py-3.5 focus:bg-white focus:border-[#7C3AED] focus:ring-4 focus:ring-[#7C3AED]/5 outline-none transition-all placeholder:text-gray-300" 
...
const CardInput = ({ icon: Icon, value, onChange, placeholder, type = "text", highlight = false }: any) => (
    <div className="relative group/input">
        ...
        <input 
            ...
            placeholder={placeholder}
            className={`w-full bg-white border border-gray-100 rounded-xl py-3 pl-10 text-sm font-semibold text-gray-700 focus:bg-white focus:border-[#7C3AED] focus:ring-2 focus:ring-[#7C3AED]/10 outline-none transition-all placeholder:text-gray-300 ${highlight ? 'text-[#7C3AED]' : ''}`}
        />
    </div>
);
